...
MNIST_NORMAL = [0, 1, 2, 3, 4]
MNIST_NOVEL = [5, 6, 7, 8, 9]
...
def torch_mnist(batch_size=32, include_novel=False, shuffle=True):
    ...
    Returns:
        (DataLoader, DataLoader, DataLoader): MNIST train, validation, and test sets.
                                              ...
                                              with values 0-1.
    """
    class_splits = {key: [.9, .1] for key in MNIST_NORMAL}
    if include_novel:
        class_splits.update({key: [0, 1] for key in MNIST_NOVEL})
    ...
    if not include_novel:
        test_set = dataset_transforms.filter_dataset(test_set, MNIST_NORMAL)
    else:
        test_set = dataset_transforms.filter_dataset(test_set, MNIST_NORMAL + MNIST_NOVEL)
