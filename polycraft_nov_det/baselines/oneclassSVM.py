...
def fit_oneclassSVM(feature_extractor, train_loader, valid_loader):
    ...
    p = ax2[0].pcolor(X, Y, acc_nn, vmin=abs(acc_nn).min(), vmax=abs(acc_nn).max())
    ax2[0].set_xscale('log')
    ax2[0].set_xlabel('gamma')
    ax2[0].set_ylabel('nu')
    ax2[0].title.set_text('Accuracy non-novel')
    cb = fig2.colorbar(p, ax=ax2[0])

    p = ax2[1].pcolor(X, Y, acc_n, vmin=abs(acc_n).min(), vmax=abs(acc_n).max())
    ax2[1].set_xscale('log')
    ax2[1].set_xlabel('gamma')
    ax2[1].set_ylabel('nu')
    ax2[1].title.set_text('Accuracy novel')
    cb = fig.colorbar(p, ax=ax2[1])
...
if __name__ == '__main__':
    ...
    classifier_path = Path("polycraft_nov_det/models/VGGPretrained_class_normal_fence_item_anvil_item_sand_item_coal_block/2022.05.10.11.51.34/1000.pt")
