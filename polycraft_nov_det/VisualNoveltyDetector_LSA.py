...
class VisualNoveltyDetector:
    ...
    def apply_model_and_determine_novelty_judgement(self, img):
        ...
        ecdf_no_novelty = load_ecdf('../models/polycraft/saved_statedict_polycraft_scale_0_75/ecdf_LSA_polycraft_no_est_075_980.npy')
        recDet = ReconstructionDet(self.model, ecdf_no_novelty) 
        
        x = preprocess_image(img, self.scale_factor, self.p_size)
        x =  torch.flatten(x, start_dim=0, end_dim=1).float()
        
        
        novelty_score = recDet.is_novel(x, 0.99)
